...
void simple() {
    ...
    try {
        Loud a{"Ala"};
        throw 15;         // type = int // zwraca do momentuu az ktos go zatrzyma
        Loud b{"Kot"};
    ...
    } catch (int a) {     // That's a match // int pasuje wiec to zostanie wyrzucone
        std::cout << "Int handler" << std::endl;
    } catch (...) {       // everything matches here, on lapie wszystko
...
int h() {
    ...
    try {
        ...
        throw; // try changing to throw e; - what happens? - puszczamy wyjatek dalej, jesli byl oby throw e to wykonana
                                                                //by zpstala kopia std::exception zaincjalizowany my expeption
    }
}
...
int k() {
    try {
        return h();
    } catch (const MyException& e) {// ten blok zotaje uzyty
